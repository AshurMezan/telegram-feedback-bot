...
const logger = require('./logger'); // Здесь пишутся логи
const db = require('./db.js'); // Это база данных
const { Bot, GrammyError, HttpError } = require('grammy'); // Это объекты для работы бота и рассылки сообщений из чата
...
const bot = new Bot(process.env.BOT_API_KEY); // Сюда пишем ключ бота
const CHAT_ID = process.env.CHAT_ID; // Это номер нашего чата
...
bot.on("message").filter(async (ctx) => {
    await sendNewsHandler(ctx, db, bot, CHAT_ID, logger, GrammyError, HttpError);
});
